...
let newBtnGroup = {
    ...
    columns: {
        ...
        get visible() {
            return (iName != "inmemdb" && (!enableCardsUi ? true : false) && (isListView ? (tstFields.length > 0) : true));
        },
        get value() {
            if (!isListView || $("#hdnListViewFieldsJSON").val() == "") {
    ...
    ivirDatatable = $(ivirTable).dataTable();
        ...
        $(".dataTables_scrollBody").on("scroll", delay(function (e) {
            ...
            if (st > lastScrollTop) { //append rows to grid only if vertical scroll bar is moved
                ...
                var scrollAtTheEnd = scrollBarExists ? $o[0].scrollHeight - $o[0].scrollTop - $o[0].clientHeight < 1 : $o[0].scrollHeight - Math.round($o.scrollTop()) < $o.outerHeight();
...
function scrollToLastKnownDrilldown(){
    setTimeout(() => {
        ...
        try {
            ...
            var loadedScroll = JSON.parse(localStorage["drilldownScrollInfo-" + appUrl] || "{}")?.[iName];

            if(loadedScroll > 0 && scrollHeight >= loadedScroll){
...
function generateCellCheckbox({ td, cellData, rowData, row, col, colID, rowDataAccess }) {
    ...
    returnString += `<div class="form-check form-check-sm form-check-custom ms-2"><input class="form-check-input ${isChkBox == "true" ? "" : "d-none"}" name="chkItem" type="checkbox" onclick="javascript:ChkHdrCheckbox();" ${isChecked ? ` checked="checked" ` : ``} value="${checkboxVal}">${listViewData}</div>`;
...
function generateCellHyperlink({ td, cellData, rowData, row, col, colID, rowDataAccess }) {
    ...
    if (returnString.startsWith("<a ")) {
        ...
        returnString =  $(`<dummy>${returnString}</dummy>`).find("a").eq(0).attr("onclick", commonOnClick).attr("data-data", cellData).attr("data-row", row).attr("data-col", FieldName.indexOf(rowDataAccess)).parents("dummy").html();
    }
...
function clickOnDemand(elem, open = true, cellData, row, col){
                    ...
                    case "t":
                        {
                            ...
                            if (isPopup) {
                                ...
                                returnString = "<a href='javascript:void(0)' onclick='javascript:setListViewDictionary(viewNavigationData[\"" + colID + "\"], " + (row + 1) + ");SetColumnName(\"" + _lastMapCol + "\",\"" + parseInt(row, 10) + "\",\"" + true + "\");LoadTstFrmIview(\"" + "./" + fullNavigationData + "\",\"" + hLinkData + "\",\"" + _urlProp + "\");' data-url=\"" + fullNavigationData + "\" " + className + " >" + (listViewCbCellData ? $(td).html() : cellData) + "</a>";
    ...
    try {
        ...
        localStorage["drilldownScrollInfo-" + appUrl] = JSON.stringify({
            ...
            [iName]: Math.round($(".dataTables_scrollBody").scrollTop())
        });
    } catch (ex) {}
...
function smartStepper(newViewTab, viewTabName, viewTabCaption) {
    ...
    <label class="input-group gap-5 w-600px ${!isListView ? `d-none` : ``}">
        ...
        <label class="form-label col-form-label text-gray-500 fst-italic">set as</label>
        <div class="form-check form-switch form-check-custom form-check-solid--- px-1">
            <label for="asDefaultView" class="form-check-label form-label col-form-label opacity-100 noempty nopurpose col-form-label-sm fs-6">
                Default View
            </label>
            <input type="checkbox" id="asDefaultView" title="" style="" name="asDefaultView" class="form-check-input opacity-100 tem custInpChk w-50px h-30px ms-2" ${ivirMainObj?.groupName === currView && currView != "" && !newViewTab ? ` checked="checked" ` : ``} />
        </div>
    </label>
    `;
    ...
    let myModal = new BSModal("newViewTabId", "", smartViewStepperHTML, () => {
        ...
        try {
            ...
            !newViewTab && $("select#viewAplyTo").prop('disabled', true).next(".select2").find(".select2-selection").addClass("bg-white");
...
<label class="input-group gap-5 ${!hasBuildAccess ? `d-none` : ``}">                    
    <label class="form-label col-form-label text-gray-500 fst-italic">Design for</label>
    <select class="form-select form-select-white border-bottom dialogSlctFld" id="viewAplyTo" placeholder="Apply To">
        <option value=${callParentNew("mainUserName")}>${callParentNew("mainUserName")}</option>
        <option value="ALL">ALL</option>
    </select>
...
function createAxRulesPopup(modalBodyLink, delayLoad = false) {
    ...
    try {
        ...
        var iFrameModalBody = `<iframe id="${modalId}" name="${modalId}" class="col-12 flex-column-fluid w-100 h-100 p-0" src="${delayLoad ? "" : modalBodyLink}" frameborder="0" allowtransparency="True"></iframe>`;
        ...
        let myModal = new BSModal(modalId, "", iFrameModalBody,
            ...
            (closing) => {
                ...
                if (isAxPop && (window.document.title == "Iview" || window.document.title == "List IView")) {
