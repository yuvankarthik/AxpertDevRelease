...
//        $.ajax({
//            url: 'signin.aspx/WindowsBtnClick',
//            type: 'POST',
//            cache: false,
//            async: true,
//            data: JSON.stringify({      
...
function ForgotPwd() {
    ...
    //$(document).on("focusout", ".login-wrapper #txtMl", function () {
    //    $('#txtMl').parents().find('.field-wrapper').addClass('hasValue');
    //})
    return false;
}
    ...
    if ($j('#axLanguage').length) {
        ...
        if (arrLangs.length < 1) {
           ...
           // $j('#axLanguage').val("English");
            ...
            $j('#hdnLangs').val(capitalizeFirstLetter($j('#axLanguage').val().trim()));
          //  createLanguageAutoComplete();
            ...
            if (localStorageLang != "" && arrLangs.indexOf(localStorageLang.toLowerCase()) > -1 && $j('#axLanguage').length > 0) {
                $j("#axLanguage").val(localStorageLang);
            } else {
                $j('#axLanguage').val(arrLangs[0]);
            }
        }
       // $j('#axLanguage').val(capitalizeFirstLetter($j('#axLanguage').val().trim()));
    }
}
...
$j(document).ready(function () {
    ...
    //    data: arrLangs
    //})
    //$j('#axLanguage').val(capitalizeFirstLetter($j('#axLanguage').val().trim()));$j("#hdnLangs").val()
   // $j('#hdnLangs').val(capitalizeFirstLetter($j('#axLanguage').val().trim()));
...
function GetProjCopyRightTxt() {
    if (isPowerBy == "") {
        $.ajax({
            ...
            data: '{name: "' + $j('#axSelectProj').val() + '", lang: "' + $j('#axLanguage').val() + '" }',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
...
function setProjectNeeds(proj, addLocalStorage = true){
    ...
    if(gsLink && gsLink.data("proj") != proj){
        // gsLink.replaceWith("<link id=\"customGlobalStyles\" data-proj=\""+ proj +"\" href=\"../"+ proj +"/customGlobalStyles.css?v="+ (new Date().getTime()) +"\" rel=\"stylesheet\" />");
            ...
            success: function (data) {
                if (data.d != "") {
                    ...
                    if (jVal != "")
                    {
                        ...
                        $j("#axPassword").val(jVal.KeepMeAutoPwd);
