...
function createIvir(jsonString) {
    ...
    filteredColumns = FieldName.filter(function (a, b, c) {
        ...
        GridView2THtml += `<th id="GridView1_ctl01_rowno" data-header-name="rowno" scope="col" style="width:${findGetParameter("tstcaption") == null ? minCellWidth : listViewCheckBoxSize}px; ">
            ...
            <div class="form-check form-check-sm form-check-custom ${appGlobalVarsObject._CONSTANTS.compressedMode ? "px-2" : "px-3 py-3"}"><input class="form-check-input ${isChkBox == "true" ? "" : "d-none"}" name="chkall" id="chkall" onclick="javascript:CheckAll();" type="checkbox" /></div>
        </th>`;
...
function createPlSmartSelect(paramName) {
    ...
    PlSmartSelect.select2({
        ...
        if (curDropdown.find(".select2-search").find(".plAdvanceSearch").length == 0) {
            curDropdown.find(".select2-search").addClass("d-flex input-group").append(`<span class="input-group-text rounded-circle btn btn-icon btn-white btn-color-gray-500 btn-active-primary shadow-sm ms-2 plAdvanceSearch" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-dismiss="click" data-bs-original-title="Advance Search" onclick="javascript:CallSearchOpen(${$(paramName)[0].id});"><span class="material-icons material-icons-style cursor-pointer">manage_search</span></span>`).find(".select2-search__field").addClass("form-control w-auto");
        }
    });
}
