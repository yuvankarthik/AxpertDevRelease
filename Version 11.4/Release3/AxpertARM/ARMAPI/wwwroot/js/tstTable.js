...
function GetDBProcTableJson(procName) {
    ...
    try {
        $.ajax({
            ...
            async: false,
            data: JSON.stringify({
                procName: procName, tstDId: callParentNew("tstDataId"), fldName: $("#hdnfieldId").val(), ChangedFields: parent.ChangedFields, ChangedFieldDbRowNo: parent.ChangedFieldDbRowNo, ChangedFieldValues: parent.ChangedFieldValues, DeletedDCRows: parent.DeletedDCRows
            }),
            dataType: 'json',
            contentType: "application/json",
            success: function (data) {
...
function AxAfterTableLoad() {
    let customCopyHtml = `
    <span class="input-group-text shift-copy selection-disabled"><span
        class="material-icons material-icons-style cursor-pointer fs-4 fldCustTableIcon"
        data-clk="shiftmap_detail000F1-tbl">file_copy</span>
        <div data-kt-menu="true"
            class="shift-option menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-title-gray-700 menu-icon-muted menu-active-bg menu-state-primary fw-bold py-4 fs-6 w-200px initialized">            
            <div class="menu-item px-3" title="Apply to Left">
                <a href="javascript:void(0);" class="menu-link d-flex px-5 " onclick="copyValue(this, 'Left')">
                    <span class="material-icons material-icons-style material-icons-1">west</span> 
                </a>
            </div>
            <div class="menu-item px-3" title="Apply to Top">
                <a href="javascript:void(0);" class="menu-link d-flex px-5 " onclick="copyValue(this, 'Top')">
                    <span class="material-icons material-icons-style material-icons-1">north</span> 
                </a>
            </div>
            <div class="menu-item px-3" title="Apply to Bottom">
                <a href="javascript:void(0);" class="menu-link d-flex px-5 " onclick="copyValue(this, 'Bottom')">
                    <span class="material-icons material-icons-style material-icons-1">south</span> 
                </a>
            </div>
            <div class="menu-item px-3" title="Apply to Right">
                <a href="javascript:void(0);" class="menu-link d-flex px-5 " onclick="copyValue(this, 'Right')">
                    <span class="material-icons material-icons-style material-icons-1">east</span> 
                </a>
            </div>
        </div>
    </span>`;

    $("#fldTable thead tr").prepend(`<th class="th-checkbox"><div style="text-align:center"><input type="checkbox" onchange="selectAll(this);"></div></th>`);
    
    let $trs = $("#fldTable tbody tr")
    $trs.each(function (idx) {
        $($trs[idx]).find('td').slice(1).append(customCopyHtml);
        $($trs[idx]).prepend(`<td class="td-checkbox"><div style="text-align:center"><input type="checkbox" onchange="enableDisableRow(this);"></div></td>`);

        let colIdx = 0;
        let $selects = $($trs[idx]).find('select');
        $selects.each(function (selIdx) {
            $($selects[selIdx]).attr("data-colno", colIdx);
            colIdx++;
        })
    });    

    $('thead th').each(function () {
        headerDateArray.push($(this).text().trim().split(" ")[0]);
    });
    headerDateArray = headerDateArray.slice(1);

    $('<div class="badge  me-2 shadow-sm pull-left Shift " style="background-color:#ae34d0" >Holiday</div><div class="badge   me-2 shadow-sm pull-left " style="background-color:#369d21" >Leave</div>').insertBefore('table#fldTable');

    fetchHolidays();
}

function enableDisableRow(elem) {
    let $row = $(elem).closest('tr');
    if (elem.checked) {
        $row.find('.selection-disabled').addClass('selection-enabled').removeClass('selection-disabled');
    }
    else {
        $row.find('.selection-enabled').addClass('selection-disabled').removeClass('selection-enabled');
    }
}

function selectAll(elem) {
    if (elem.checked) {
        $("#fldTable tbody .td-checkbox input[type='checkbox']").prop("checked", true).trigger("change");
    }
    else {
        $("#fldTable tbody .td-checkbox input[type='checkbox']").prop("checked", false).trigger("change");
    }
}

function copyValue(elem, direction) {
    let $currentElem = $(elem).closest('td').find('select');
    let currCol = $currentElem.attr("data-colno");
    let currVal = $currentElem.attr("value");
    if (direction.toLowerCase() == "right") {
        let $row = $(elem).closest('tr');
        let $selects = $row.find("td select");
        $selects.each(function (idx) {
            if (currCol < idx && idx > 0) {
                $($selects[idx]).append('<option value="' + currVal + '" selected="selected">' + currVal + '</option>');
                $($selects[idx]).attr("value", currVal);
            }
        });
    }
        ...
        $("tr").each(function () {
            ...
            if (row_id < id) {
                if ($(this).find('.td-checkbox input').prop('checked') == true) {
                    $("tr#" + row_id + " select[data-colno='" + currCol + "']").append('<option value="' + currVal + '" selected="selected">' + currVal + '</option>');
                    $("tr#" + row_id + " select[data-colno='" + currCol + "']").attr("value", currVal);
                }
            }
        })
    } else if (direction.toLowerCase() == "bottom") {
        let id = $(elem).parents('tr').attr('id');
        $("tr").each(function () {
            let row_id = $(this).attr('id');
            if (row_id > id) {
                if ($(this).find('.td-checkbox input').prop('checked') == true) {
...
function constructShiftJson() {
    var resultArray = [];
    var employeeCode = '';

    $("tbody tr td select").each(function () {
        let colNo = $(this).attr("data-colno");
        if (colNo == "0") {
            employeeCode = this.value;
        } else {
            resultArray.push({ "employee_code": employeeCode, "shift_date": headerDateArray[colNo], "shift_time": this.value })
        }
    })
...
function fetchHolidays() {
    let fromDate = parent.$("#from_date000F1").val();
    let toDate = parent.$("#to_date000F1").val();
    if (fromDate == "" || fromDate.indexOf('d') > -1 || !parent.isDate(fromDate))
        return;
    if (toDate == "" || toDate.indexOf('d') > -1 || !parent.isDate(toDate))
        return;

    fromDate = moment(parent.GetDateObj(fromDate)).format('DD-MMM-YYYY');
    toDate = moment(parent.GetDateObj(toDate)).format('DD-MMM-YYYY');

    let employeeCodes = parent.GetFieldValue("employee_code_list000F1");
    employeeCodes = employeeCodes.split(',');
    employeeCodes = employeeCodes.map(element => {
        return `'` + element + `'`;
    });
    ...
    $.ajax({
        type: "POST",
        url: "../custompages/aspx/TreeConfig_v2.aspx/GetHolidays",
        cache: false,
        async: false,
        data: JSON.stringify({ fromDate: fromDate, toDate: toDate, employeeCodes: employeeCodes }),
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        success: function (data) {
            try {
            ...
            let tempDateArr = headerDateArray.map(element => {
                return element.toUpperCase();
            });

            let $emps = $('select[data-colno="0"]');
            let empArr = [];
            $emps.each(function (idx) {
                empArr.push($($emps[idx]).val());
            })
            ...
            data.forEach(function (row) {
                let date = row.holiday_date.toUpperCase();

                let dateIdx = tempDateArr.indexOf(date);

                if (dateIdx > -1) {
                    let empIdx = empArr.indexOf(row.employee_code);
                    if (empIdx > -1) {
                        let leaveColor = '';
                        let cellClass = '';
                        if (row.type.toUpperCase() == "HOLIDAY") {
                            leaveColor = "#ae34d0";
                            cellClass = "shift-holiday";
                        }
                        else {
                            leaveColor = "#369d21";
                        ...
                        $("tr").eq(empIdx).find("select[data-colno='" + dateIdx + "']").closest('td').css("background-color", leaveColor);
                        $("tr").eq(empIdx).find("select[data-colno='" + dateIdx + "']").closest('td').addClass(cellClass)
                    }
                }
...
function AxAfterAddTableData() {
    let json = constructShiftJson();
    let fldId = "selected_shift_data_json000F1";
    $(callParentNew(fldId, "id")).val(json);
    UpdateArray(fldId, json);
}
