...
$j(document).ready(function () {
    ...
    if (typeof gridFixedHeader != "undefined" && gridFixedHeader == "true") {
        ...
        // $('.griddivColumn ').addClass('gridFixedHeader overflow-auto').css({"max-height" : "calc(100vh - 130px)"});
        // $(".gridFixedHeader table thead tr th").addClass("bg-white position-sticky top-0 z-index-1")//.css({"z-index" : "4"});

        /* 
        [id^=colScroll].gridFixedHeader {
            overflow: auto;
            max-height: calc(100vh - 130px);  
        }

        [id^=colScroll].gridFixedHeader table thead tr th {
            background: #fff;
            position: sticky;
            top: 0;
        }
    ...
    if (appGlobalVarsObject._CONSTANTS.compressedMode) {
        if (typeof dvId != "undefined" && dvId != "") {
            ...
            if (typeof dcLayoutType == "undefined" || dcLayoutType == "" || dcLayoutType == "default")
                ...
                fcwidth = fcwidth / 10;
                fcwidth = 12 - fcwidth;
                $(".compressedModeUI .input-group").wrap(`<div class='col-sm-${fcwidth}'></div>`);
        ...
        $(".content [id^=DivFrame] .form-switch .form-check-input").removeClass("w-40px").addClass("w-60px");
        ...
        if (typeof dcLayoutType != "undefined" && (dcLayoutType == "signle" || dcLayoutType == "double" || dcLayoutType == "triple")) {
            let fcwidth = parseInt(designObj[0].fieldCaptionWidth);
            fcwidth = fcwidth / 10;
            fcwidth = 12 - fcwidth;
            $(".content .input-group").wrap(`<div class='col-sm-${fcwidth}'></div>`);
            $(".upload-button").addClass('mt-4');
            $(".fldImageCamera").removeClass('mt-n4').addClass('mt-0');
            if (!staticRunMode)
                $(".colFldGridStackWidth").addClass('mb-3');
...
function ProcessScriptFormControl(listControls, actionStr, sfName) {
    ...
    listControls.split(',').forEach(function (contName) {
        ...
        isFieldBtn = false;
        var destfld = $j("#" + contName + ":not(.tstformbutton,.axpBtnCustom,.dwbIvBtnbtm,.menu-link)");
        if (destfld.length == 0) {
...
function DropzoneInit(dvId) {
    ...
    _thisDiv.each(function () {
        ...
        myDropzone.on("addedfile", function (file) {
            ...
            this.options.url = url + `TstFileUpload.ashx?thisFld=${$(id).attr("id").substr(9)}&filePath=${$(".axpFilePath_" + $(id).attr("id").substring(("dropzone_axpfile_").length)).val() == undefined ? "" : $(".axpFilePath_" + $(id).attr("id").substring(("dropzone_axpfile_").length)).val()}&dcNo=${GetFieldsDcNo($(id).attr("id"))}&attFldName=${funame}`;
        ...
        myDropzone.on("complete", function (progress) {
            ...
            if (progress.status == 'success') {
                if (progress.xhr.response == "success" || progress.xhr.response.startsWith("success:")) {
                    ...
                    if (fuInpVal == "")
                        fuInpVal = progress.name;
                    else {
                        const dupFileObj = fuInpVal.split(',').filter(file => file == progress.name)?.[0];
                        if (dupFileObj) {
                            myDropzone.removeFile(progress);
                            return false;
                        } else
                            fuInpVal = fuInpVal + "," + progress.name;
                    }
...
function rtfCkeditorAlignment() {
    ...
    for (var B = 0; B < A.length; B++) {
        if (A[B].id.startsWith("rtf_") || A[B].id.startsWith("rtfm_") || A[B].id.startsWith("fr_rtf_") || GetDWBFieldType(GetFieldsName(A[B].id)) == "Rich Text" || GetDWBFieldType(GetFieldsName(A[B].id)) == "Large Text") {
...
$j(document).on("click", ".gridHeaderSwitch", function (e) {
